...
class _MyAppState extends State<MyApp> {
    ...
    Stream.fromFuture(getTemporaryDirectory())
        .flatMap((tempDir) {
          ...
          if (exists) {
            ...
            return Stream.fromFuture(rootBundle.load("images/$filename")).flatMap((bytes) =>
                Stream.fromFuture(qrCodeFile
                    .writeAsBytes(bytes.buffer.asUint8List(bytes.offsetInBytes, bytes.lengthInBytes))));
          }
        })
        .flatMap((file) => Stream.fromFuture(QrCodeToolsPlugin.decodeFrom(file.path)))
        .listen((data) {
          setState(() {
            _data = data;
          });
  ...
  void _getPhotoByGallery() {
    Stream.fromFuture(picker.pickImage(source: ImageSource.gallery)).flatMap((file) {
      if (file == null) {
        return Stream.value('');
      }
      setState(() {
